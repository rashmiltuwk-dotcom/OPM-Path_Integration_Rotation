classdef OptiTrackBridge
    ...
    % ================================================================
    % PURPOSE:
    %   - Calls Motive's native startRecord() function
    %   - If successful, captures the EXACT system time when recording started
    %   - Stores this as MotiveT0 (the global reference zero point)
    %   - All subsequent timestamps (task traces, continuous data) are 
    %     subtracted by MotiveT0 to make them relative to this moment
    %
    % BEHAVIOR IF RECORDING WORKS:
    %   - MotiveT0 is set → all traces use it as reference zero
    %   - Task traces in StopRecording() subtract MotiveT0
    %   - Continuous data in SaveContinuous() subtracts MotiveT0
    %   - Result: all timestamps synchronized to Motive's recording start
    %
    % BEHAVIOR IF RECORDING FAILS:
    %   - MotiveT0 stays at 0 → no effect on timestamps
    %   - Experiment continues normally
    %   - Data collected but not time-synced to Motive
    % ================================================================
    
        global OP_BRIDGE_STATE
    
        if OP_BRIDGE_STATE.IsDummy
            OP_BRIDGE_STATE.MotiveT0 = GetSecs();
            return;
        end
    
        % Call Motive's native startRecord
        success = OP_BRIDGE_STATE.NatNetClient.startRecord();
    
        % If it succeeds, capture the moment as reference zero
        if success
            OP_BRIDGE_STATE.MotiveT0 = GetSecs();
            fprintf('>>> Motive recording STARTED. MotiveT0 = %.6f\n', OP_BRIDGE_STATE.MotiveT0);
        else
            fprintf('>>> WARNING: startRecord() failed. Recording may not be active.\n');
        end
    end
...
;
        ...
        function [distWalkedTorso, distWalkedHead, headTrace, torsoTrace] = WaitForWalkEnd(headID, torsoID, win, tolerance)
            ...
            % --- CAPTURE INITIAL POSITION WHEN DATA IS VALID (NON-NaN) ---
            % Match polling version: just wait for non-NaN position data
            % Error thresholds are enforced in GetOpti() during main loop
            global OP_DATA_BUFFER
            validStart = false;
            while ~validStart
                idx = OP_DATA_BUFFER.idx - 1;
                if idx >= 1
                    hx = OP_DATA_BUFFER.hx(idx);
                    tx = OP_DATA_BUFFER.tx(idx);
                    if ~isnan(hx) && ~isnan(tx)
                        startHeadPos  = [OP_DATA_BUFFER.hx(idx), OP_DATA_BUFFER.hy(idx), OP_DATA_BUFFER.hz(idx)];
                        startTorsoPos = [OP_DATA_BUFFER.tx(idx), OP_DATA_BUFFER.ty(idx), OP_DATA_BUFFER.tz(idx)];
                        validStart = true;
                    end
                end
                WaitSecs(0.01);
        ...
        function [startHead, startTorso, finalHead, finalTorso, headTrace, torsoTrace] = WaitForRotationDual(headID, torsoID, targetDeg, win, dirCode)
            ...
            % --- CAPTURE INITIAL ORIENTATION WHEN DATA IS VALID (NON-NaN) ---
            % Match polling version: just wait for non-NaN euler data
            % Error thresholds are enforced in GetOpti() during main loop
            global OP_DATA_BUFFER
            validStart = false;
            while ~validStart
                idx = OP_DATA_BUFFER.idx - 1;
                if idx >= 1
                    hyaw = OP_DATA_BUFFER.hyaw(idx);
                    tyaw = OP_DATA_BUFFER.tyaw(idx);
                    if ~isnan(hyaw) && ~isnan(tyaw)
                        startHeadEuler = [OP_DATA_BUFFER.hroll(idx), OP_DATA_BUFFER.hpitch(idx), OP_DATA_BUFFER.hyaw(idx)];
                        startTorsoEuler = [OP_DATA_BUFFER.troll(idx), OP_DATA_BUFFER.tpitch(idx), OP_DATA_BUFFER.tyaw(idx)];
                        validStart = true;
                    end
                end
                WaitSecs(0.01);
